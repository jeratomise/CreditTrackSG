...
const DISMISSED_KEY = 'credittrack_sharebar_dismissed';
...
export const FloatingShareBar: React.FC = () => {
  ...
  return (
    // Sits in normal flow, not fixed: as a fixed z-50 bar it covered the mobile
    // header underneath it, including the avatar button that opens the account menu.
    <div className="relative z-30 bg-marine-900 border-b border-brass-500/20" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
      <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between gap-3">
        {/* Left: CTA text */}
        <div className="flex items-center gap-2.5 min-w-0">
          <Share2 className="w-4 h-4 text-brass-400 shrink-0" strokeWidth={1.5} />
          <p className="text-sm text-ink-soft truncate">
            Share CreditTrack, earn free Pro months
          </p>
          {stats.referralCode && (
            <span className="hidden sm:inline font-mono text-[10px] uppercase tracking-[0.14em] text-brass-400 shrink-0">
              {stats.referralCode}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-1 shrink-0">
          {shareLinks.map((link) => (
            <a
              ...
              className="flex items-center justify-center w-11 h-11 text-ink-mute hover:text-brass-400 transition-colors duration-150"
              title={link.name}
            >
              {link.icon}
            </a>
        ...
        <button
          ...
          aria-label="Dismiss"
          className="flex items-center justify-center w-11 h-11 text-ink-mute hover:text-brass-400 transition-colors duration-150 shrink-0"
        >
