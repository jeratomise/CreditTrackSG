...
export const FloatingShareBar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const dismissedKey = 'credittrack_sharebar_dismissed';
  ...
  const shareText = `Track your credit card bills the smart way — AI-powered with miles optimization! Sign up with my referral code: ${stats.referralCode || ''} ${stats.referralUrl || ''}`;
  ...
  return (
    <div className="fixed top-0 inset-x-0 z-50 bg-gradient-to-r from-indigo-950 via-indigo-900 to-indigo-950 shadow-xl shadow-indigo-950/20">
      {/* Ambient glow */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-[-60%] left-1/2 -translate-x-1/2 w-96 h-40 bg-indigo-500/10 rounded-full blur-3xl" />
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <p className="text-white text-sm font-bold whitespace-nowrap">
            Share CreditTrack& Earn Rewards!
          </p>
          <span className="hidden sm:inline text-indigo-300 text-xs">
            {stats.referralCode ? `Use code ${stats.referralCode}` : ''}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {shareLinks.map((link) => (
            <a
              ...
              className={`flex items-center justify-center w-8 h-8 rounded-lg text-white transition-all ${link.bg}`}
              title={link.name}
            >
              {link.icon}
            </a>
        ...
        <button
          onClick={() => {
            setDismissed(true);
            localStorage.setItem('credittrack_sharebar_dismissed', '1');
          }}
          className="flex items-center justify-center w-7 h-7 rounded-lg text-indigo-400 hover:text-white hover:bg-indigo-800 transition-all shrink-0"
          title="Dismiss"
        >
          <X className="w-4 h-4" />
        </button>
